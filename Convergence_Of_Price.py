...
def mainCalculation():
    ...
    S = Paths["S"]
    payoff = np.maximum(S[:,-1] - K, 0)
    value1 = np.mean(payoff)*(np.exp(-r * T))
    value2 = BS_Call_Option_Price(S_0,K,sigma, T,r)
...
paths_list = [100, 500, 1000, 2500, 5000, 10000, 25000, 50000]
...
for n_paths in paths_list:
    ...
    mc_price, analytical = mainCalculation()

    price_diff = mc_price - analytical

    results.append([n_paths, analytical, mc_price, price_diff])
